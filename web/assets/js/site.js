$(function(){
    ...
    var process_url = 'upload'; //PHP script
    ...
    //initialize blueimp fileupload plugin
        ...
        if (data.result.message) {
            ...
            $(data.context.children()[0]).addClass('file-uploaded');
            $(data.context.children()[0]).find('canvas').wrap(link);
            $(data.context.children()[0]).find('.file-remove').hide(); 
            var done = $('<span class="text-success"/>').text(data.result.message);
            $(data.context.children()[0]).append(done);
